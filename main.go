...
func main()  {
	...
	// функцию "home" регисрируется как обработчик для URL-шаблона "/"
	mux := http.NewServeMux()
	mux.HandleFunc("/", home)

	// Используется функция http.ListenAndServe() для запуска нового-сервера
	// Мы передаемм два параметра: TCP-адрес сети для прослушивания (в данном случае это "localhost:4000")
	// и созданный рутер. Если вызов http.ListenAndServe() возвращает ошибку
	// мы используем функцию log.Fatal() для логированя ошибок. Обратите внимание
